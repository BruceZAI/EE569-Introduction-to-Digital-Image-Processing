...
RwarpedForky = dewarping('Forky_warp.raw', 328, 328);
myWriteRaw(warpedForky, 'Forky_dewarp.raw')
Rwarped22 = dewarping('22_warp.raw', 328, 328);
...
for i=1:height
    for j=1:width
        x = (i - (height/2 - 1)) / (height/2 - 1);
        y = (j - (width/2 - 1)) / (width/2 - 1);
        [u,v] = ori2mod(x,y);
        p = uint32((u * (height / 2 - 1)) + ((height / 2) - 1));
        q = uint32((v * (width / 2 - 1)) + ((width/2) - 1));
        G2(p,q,1) = G(i, j, 1);
        G2(p,q,2) = G(i, j, 2);
        G2(p,q,3) = G(i, j, 3);
    end
