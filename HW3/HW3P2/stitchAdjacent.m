function [img_res,H,num_inliers,residual] = stitchAdjacent(img_src, img_des)
    ...
    sift_radius = 5;
    descriptors_src = sift(gd_src,[x_src,y_src,sift_radius*ones(length(x_src),1)],1.5);
        
    descriptors_des = sift(gd_des,[x_des,y_des,sift_radius*ones(length(x_des),1)],1.5);
        
    
    num_putative_matches = 200;
    [matches_src,matches_des] = putativeMatches(descriptors_src,descriptors_des,num_putative_matches);
        
    XY_src = [x_src(matches_src),y_src(matches_src)];
    XY_des = [x_des(matches_des),y_des(matches_des)];
%     length(XY_src)
    num_ransac_iter = 5000;
    [H,num_inliers,residual] = ransac(XY_src,XY_des,num_ransac_iter,@homoFit,@homoTransf);
